...
WHERE movie.id IN (SELECT movieid FROM casting WHERE actorid = (SELECT id FROM actor WHERE name = 'Julie Andrews')) AND ord = 1;
...
JOIN casting ON movie.id = movieid
JOIN actor ON actor.id = actorid
WHERE movie.id IN (SELECT movieid FROM casting WHERE actorid = (SELECT id FROM actor WHERE actor.name = 'Art Garfunkel')) AND name <> 'Art Garfunkel'
ORDER BY name;